...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $user['role'] === 'teamlead') {
    ...
    if ($postAction === 'create_team') {
        ...
        $teamDesc = trim($_POST['team_desc'] ?? '');
        if ($teamName) {
            $stmt = $db->prepare("INSERT INTO teams (name, description) VALUES (?, ?) RETURNING id");
            $stmt->execute([$teamName, $teamDesc]);
            $newTeamId = $stmt->fetchColumn();
            // Добавляем тим лида в команду
            $db->prepare("INSERT INTO team_members (user_id, team_id) VALUES (?, ?) ON CONFLICT DO NOTHING")
               ->execute([$user['id'], $newTeamId]);
            $msg = '✅ Команда «' . htmlspecialchars($teamName) . '» создана';
        }
    }
    ...
    if ($postAction === 'edit_team') {
        ...
        $teamName = trim($_POST['team_name']  ?? '');
        $teamDesc = trim($_POST['team_desc']  ?? '');
        // Проверяем что тим лид состоит в этой команде
        $check = $db->prepare("SELECT 1 FROM team_members WHERE user_id = ? AND team_id = ?");
        $check->execute([$user['id'], $tid]);
        if ($check->fetchColumn()) {
            $db->prepare("UPDATE teams SET name=?, description=? WHERE id=?")
               ->execute([$teamName, $teamDesc, $tid]);
...
<section class="page-content">
    <div class="container">
        <div class="section-header">
            <h2>👥 <?php echo $user['role'] === 'teamlead' ? 'Мои команды' : 'Моя команда'; ?></h2>
